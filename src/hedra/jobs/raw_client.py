...
class RawJobsClient:
    ...
    def get_status(
        ...
        *,
        ...
        """
        _response = self._client_wrapper.httpx_client.request(
            f"jobs/{encode_path_param(job_id)}/status",
            method="GET",
            params={
    ...
    def list_job_logs(
        ...
        *,
        ...
        """
        _response = self._client_wrapper.httpx_client.request(
            f"jobs/{encode_path_param(job_id)}/logs",
            method="GET",
            params={
                "limit": limit,
                "cursor": cursor,
        ...
        try:
            if 200 <= _response.status_code < 300:
                _parsed_response = typing.cast(
                    JobLogListResponse,
                    parse_obj_as(
                        type_=JobLogListResponse,  # type: ignore
                        object_=_response.json(),
                    ),
                )
                _items = _parsed_response.data
                _parsed_next = _parsed_response.next_cursor
                _has_next = _parsed_next is not None and _parsed_next != ""
                _get_next = lambda: self.list_job_logs(
                    job_id,
                    limit=limit,
                    cursor=_parsed_next,
                    request_options=request_options,
        ...
        *,
        ...
        try:
            if 200 <= _response.status_code < 300:
                ...
                async def _get_next():
                    return await self.list_job_logs(
                        job_id,
                        limit=limit,
                        cursor=_parsed_next,
                        request_options=request_options,
